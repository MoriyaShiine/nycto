	...
	protected RecipeProvider createRecipeProvider(HolderLookup.Provider registries, RecipeOutput output) {
		return new RecipeProvider(registries, output) {
			...
			public void buildRecipes() {
				...
				shaped(RecipeCategory.DECORATIONS, ModItems.BLOOD_FOUNTAIN).define('S', Items.STONE_BRICK_SLAB).define('B', Items.GLASS_BOTTLE).pattern(" S ").pattern(" B ").pattern("SSS").unlockedBy("has_bottle", has(Items.GLASS_BOTTLE)).save(output);
				...
				shaped(RecipeCategory.COMBAT, ModItems.HALBERD).define('G', ConventionalItemTags.GOLD_INGOTS).define('I', ConventionalItemTags.IRON_INGOTS).define('A', Items.DIAMOND_AXE).define('S', Items.STICK).pattern(" GI").pattern(" AI").pattern("S  ").unlockedBy("has_diamond", has(ItemTags.DIAMOND_TOOL_MATERIALS)).save(output);
