...
public class ItemDescriptionsClientEvent implements ItemTooltipCallback {
	public static void init() {
		ItemTooltipCallback.EVENT.register(new ItemDescriptionsClientEvent());
	}
	...
	static {
		ARMOR_SET_BONUSES.add(stack -> getArmorSetText(stack, NyctoItemTags.VAMPIRE_ARMOR, ItemDescriptionsClientEvent.VAMPIRE_ARMOR_BONUSES));
		ARMOR_SET_BONUSES.add(stack -> getArmorSetText(stack, NyctoHunterTypes.VAMPIRE.armorTagKey, ItemDescriptionsClientEvent.VAMPIRE_HUNTER_ARMOR_BONUSES));
	}
	...
	public void getTooltip(ItemStack stack, Item.TooltipContext tooltipContext, TooltipFlag tooltipFlag, List<Component> lines) {
		...
		if (stack.is(NyctoItems.WEREWOLF_ALTAR)) {
	...
	private static Optional<List<Component>> getWeaknessText(ItemStack stack) {
		if (stack.is(NyctoItemTags.HURTS_VAMPIRES) && NyctoAPI.isVampire(client.player)) {
