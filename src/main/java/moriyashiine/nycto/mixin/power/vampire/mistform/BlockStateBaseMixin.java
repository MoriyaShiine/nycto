...
public abstract class BlockStateBaseMixin implements TypedInstance<Block> {
	...
	private VoxelShape nycto$mistForm(VoxelShape original, @Local(argsOnly = true) CollisionContext context) {
		if (original != Shapes.empty() && original != Shapes.block() && context instanceof EntityCollisionContext entityContext && entityContext.getEntity() instanceof Player player && NyctoEntityComponents.MIST_FORM.get(player).isEnabled() && !is(NyctoBlockTags.MIST_FORM_UNPASSABLE)) {
			return Shapes.empty();
		}
		return original;
	}
