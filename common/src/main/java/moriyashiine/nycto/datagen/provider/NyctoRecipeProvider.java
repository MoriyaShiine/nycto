...
public class NyctoRecipeProvider extends FabricRecipeProvider {
	...
	public static MultiRegistryBootstrap create() {
		return new MultiRegistryBootstrap() {
			...
			public void run(MultiRegistryBootstrap.BootstrapGetter registries) {
				Provider provider = new Provider(registries.get(Registries.RECIPE), registries.get(Registries.ADVANCEMENT));
	...
	protected RecipeProvider createRecipeProvider(HolderLookup.Provider registries, BootstrapContext<Recipe<?>> recipes, BootstrapContext<Advancement> advancements) {
		return new FabricProvider(recipes, advancements);
	}

	private static class FabricProvider extends Provider {
		private final NyctoBrewingProvider brewingProvider = new NyctoBrewingProvider(output);

		protected FabricProvider(BootstrapContext<Recipe<?>> recipeOutput, BootstrapContext<Advancement> advancementOutput) {
	...
	private static class Provider extends RecipeProvider {
		...
		public void buildRecipes() {
			...
			shaped(RecipeCategory.DECORATIONS, NyctoItems.POPLAR_COFFIN).define('P', Items.POPLAR_PLANKS).define('B', ItemTags.BEDS).pattern("PPP").pattern("PBP").pattern("PPP").unlockedBy("has_bed", has(ItemTags.BEDS)).save(output);
	...
	private static class NyctoBrewingProvider extends BrewingProvider {
		protected NyctoBrewingProvider(RecipeOutput output) {
			super(output);
		}
		...
		protected void buildMixes() {
			buildMix(Potions.POISON, NyctoItems.ACONITE, NyctoPotions.WITHER);
			buildMix(NyctoPotions.WITHER, Items.REDSTONE, NyctoPotions.LONG_WITHER);
			buildMix(NyctoPotions.WITHER, Items.GLOWSTONE_DUST, NyctoPotions.STRONG_WITHER);
			buildMix(Potions.HEALING, NyctoItems.GARLIC, NyctoPotions.GARLIC);
			buildMix(Potions.STRONG_HEALING, NyctoItems.GARLIC, NyctoPotions.STRONG_GARLIC);
			buildMix(NyctoPotions.GARLIC, Items.REDSTONE, NyctoPotions.LONG_GARLIC);
			buildMix(NyctoPotions.GARLIC, Items.GLOWSTONE_DUST, NyctoPotions.STRONG_GARLIC);
		}
	}
}
