...
public class Hunter extends Pillager {
	...
	public void setHunterType(HunterType hunterType) {
		getHunterType().getGoals(this).forEach((_, goal) -> getGoalSelector().removeGoal(goal));
		entityData.set(HUNTER_TYPE_ID, hunterType);
		hunterType.getGoals(this).forEach((priority, goal) -> getGoalSelector().addGoal(priority, goal));
	}
